...
export function makeTelemetry(opts = {}) {
  ...
  try {
    if (url && hasLS) {
      localStorage.setItem("zero.ws", url);
      localStorage.setItem("zero.ws.ts", String(Date.now()));
    } else if (!url && hasLS) {
      const remembered = localStorage.getItem("zero.ws");
      const ts = Number(localStorage.getItem("zero.ws.ts") || 0);
      if (remembered && Date.now() - ts < WS_MEMORY_MS) url = remembered;
      else if (remembered) { localStorage.removeItem("zero.ws"); localStorage.removeItem("zero.ws.ts"); }
    }
  } catch (_) {}
